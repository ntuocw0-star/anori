...
### Step 10 — Evidence Recovery Workflow（v1.2 新增）

**触发条件：** REL 存在 `evidence_basis_status: Missing` 或 `traceback_status: Failed`

**执行流程：**

```
Evidence Basis Missing
        ↓
Step 10a: Evidence Traceback
（查找 primary evidence 的原始依据）
        ↓
Traceback Succeeded → 补录文献，生成 EA，关闭 evidence_basis_status
        ↓
Traceback Failed → 进入 Step 10b
        ↓
Step 10b: New Evidence Search
（主动寻找新的直接证据，不是替代历史）
        ↓
Found Tier 1/2 → 生成 EA，不修改 reviewer_grade
        ↓
Stopping Rule triggered（见下） → 接受 Best Available Evidence
        ↓
Step 10c: Best Available Evidence
生成 EA，标注 Evidence Recovery context
保留 evidence_basis_status: Missing
保留 reviewer_grade 不变
```

**Evidence Recovery Stopping Rule：**

```
连续完成 ≥5 次高质量数据库检索
（每轮使用不同策略：构念驱动 / 测量工具驱动 / 文献类型驱动等）
未发现 Tier 1 / Tier 2 Direct Evidence
→ 接受最高等级可获得证据（Best Available Evidence）
→ 保留 evidence_basis_status = Missing
→ EA 中明确注明：Best Available Evidence after Evidence Recovery search
```

**验证案例：** REL-000027（EA-000016，2026-07-07，5轮检索后触发）

---

### Governance Principles（v1.2 正式写入）

以下原则经过真实案例验证，正式从 Governance Notes 升级至 Workflow：

**对称原则：**

| 情境 | 原则 |
|------|------|
| Evidence 不足时 | 不升级（Quality before Quantity） |
| Traceback 失败时 | 不降级（Traceback failure ≠ Evidence refutation） |

**完整表述：**
> Evidence Level reflects current evidence strength, not upgrade history.
> Traceback failure does not invalidate an existing evidence judgment — it means Evidence provenance is incomplete, not that the judgment is wrong.
> Missing evidence history is an integrity issue (KIR), not an evidence gap.

---
*Evidence Upgrade Workflow v1.2 — Updated 2026-07-07*
*Changes from v1.1:*
*- Step 10 新增：Evidence Recovery Workflow（Traceback → New Evidence Search → Stopping Rule）*
*- Evidence Recovery Stopping Rule 正式写入（≥5轮检索；验证案例：REL-000027 / EA-000016）*
*- Governance Principles 正式写入（两条对称原则从 OBS 升级至 Workflow）*
*Next review trigger: 任意 OBS 积累到 2 个独立案例，或发现新的重复执行模式*
